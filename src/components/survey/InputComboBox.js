...
const InputComboBox = ({ value, onChange, placeholder }) => {
    ...
    const handleInputChange = (e) => {
        const trimmedValue = e.target.value.trimStart(); // Removes leading spaces
    ...
    const handleOptionClick = (option) => {
        const trimmedOption = option.trim(); // Ensures no trailing/leading spaces
        setInputValue(trimmedOption);
        onChange(trimmedOption);
        setDropdownVisible(false);
    };

    const filteredOptions = neighborhoodNames.filter((option) =>
        option.toLowerCase().includes(inputValue.trim().toLowerCase()) // Ensures filtering is clean
    );
    ...
    return (
        <div className="relative">
            <input
                ...
                onBlur={() => setInputValue(inputValue.trim())} // Trims trailing spaces when the user leaves the input
            />
